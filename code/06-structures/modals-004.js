...
nextButtons.forEach((button) => {
    button.addEventListener("click", function (event) {
        ...
        let nextIndex = (currentModalIndex + 1) % modals.length;
        showModal(nextIndex);
    });
});
...
prevButtons.forEach((button) => {
    button.addEventListener("click", function (event) {
        event.stopPropagation();
        let prevIndex = (currentModalIndex - 1 + modals.length) % modals.length;
        showModal(prevIndex);
    });
});
...
// function showModal(index) {
//     let modals = document.querySelectorAll(".modal"); // Get all modals again
//     if (!modals.length || index < 0 || index >= modals.length) {
//         console.error("❌ Invalid modal index:", index);
//         return;
//     }
...
//     let modal = modals[index];
//     let modalBody = modal.querySelector(".modal-body");
//     let modalSrc = modal.getAttribute("data-src") || modalBody.getAttribute("data-src"); // Ensure correct path

//     // If the modal content is not yet loaded, fetch it
//     if (modalBody && modalSrc && !modalBody.hasAttribute("data-loaded")) {
//         console.log("Loading modal content from:", modalSrc);
    
//         fetch(modalSrc)
//             .then(response => response.text())
//             .then(content => {
//                 modalBody.innerHTML = content;
//                 modalBody.setAttribute("data-loaded", "true"); // Prevents multiple loads
//                 executeInlineScripts(modalBody);
    
//                 // 🚨 Ensure modal only opens when a user clicks, NOT on load
//                 if (!modal.classList.contains("open")) {
//                     console.log("✅ Modal content loaded, but NOT opening modal");
//                     return;
//                 }
//             })
//             .catch(error => console.error("Error loading modal content:", error));
...
//     console.log("Page loaded. No modals should be open.");
// });